...
`ngrok-skip-browser-warning` only works when the *client* sends it. curl, Postman and OkHttp
are all unaffected, so the Android app never sees it — only manual browser testing does.
